function onBlur(){
	...
	if(userid.length < 8){
		$('.valid').html('아이디는 8자 이상이어야 됩니다.').css({'display': 'block', 'color': 'red'});
		...
		$.get('/user/idchk/'+userid, function(r){
			if(r.code == 200){
				if(r.isUsed) {
					$('.valid').html('해당 아이디는 사용할 수 있습니다.').css({'display': 'block', 'color': 'blue'});
					$("input[name='useridValid']").val('valid');
				}
				else {
					$('.valid').html('해당 아이디는 사용할 수 없습니다.').css({'display': 'block', 'color': 'red'});
					$("input[name='useridValid']").val('');
				}
			}
			else console.log(r);
